    ...
    auto imu_callback = [&](const rs2::frame &frame) {
        ...
        if (rs2::frameset fs = frame.as<rs2::frameset>()) {
            ...
            if (profile_changed(
                ...
                // If the profile was changed, update the align object, and also get the new
                // device's depth scale
                pipe_profile = pipe.get_active_profile();
                align_to = find_stream_to_align(pipe_profile.get_streams());
                align = rs2::align(align_to);
            }

            // Align depth and rgb takes long time, move it out of the interruption to avoid losing
            // IMU measurements
            fsSLAM = fs;
            ...
            cv::Mat(cv::Size(width_img, height_img), CV_16U, (void*)(depth_frame.get_data()),
            cv::Mat::AUTO_STEP);
...
rs2_stream find_stream_to_align(const std::vector<rs2::stream_profile> &streams) {
    ...
    // with. We prioritize color streams to make the view look better. If color is not available,
    // we take another stream that (other than depth)
