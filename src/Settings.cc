...
Settings::Settings(const std::string &configFile, const int &sensor)
    ...
    if (sensor_ == System::IMU_MONOCULAR || sensor_ == System::IMU_STEREO ||
        sensor_ == System::IMU_RGBD || sensor_ == System::IMU_MULTI) {
        readIMU(fSettings);
...
ostream &operator<<(std::ostream &output, const Settings &settings) {
    ...
    if (settings.sensor_ == System::STEREO || settings.sensor_ == System::IMU_STEREO ||
        ...
        if (settings.cameraType_ == Settings::KannalaBrandt) {
            ...
            if (settings.sensor_ == System::IMU_MULTI) {
                auto vOverlapping3 =
                    static_cast<KannalaBrandt8 *>(settings.calibration3_)->mvLappingArea;
                auto vOverlapping4 =
                    static_cast<KannalaBrandt8 *>(settings.calibration4_)->mvLappingArea;
                output << "\t-Camera 3 overlapping area: [ " << vOverlapping3[0] << " , "
                       << vOverlapping3[1] << " ]" << endl;
                output << "\t-Camera 4 overlapping area: [ " << vOverlapping4[0] << " , "
                       << vOverlapping4[1] << " ]" << endl;
            }
        }
    }
