    ...
    bool bBad = false;
    {
        ...
        if (mObservations.count(pKF)) {
            ...
            int leftIndex = get<0>(indexes), rightIndex = get<1>(indexes),
                sideleftIndex = get<2>(indexes), siderightIndex = get<3>(indexes);
...
void MapPoint::SetBadFlag() {
    ...
    for (map<KeyFrame *, tuple<int, int, int, int>>::iterator mit = obs.begin(), mend = obs.end();
         mit != mend;
         mit++) {
...
void MapPoint::UpdateNormalAndDepth() {
    ...
    if (pRefKF->NLeft == -1) {
        ...
        level = pRefKF
                    ->mvKeysSideRight
                        [siderightIndex - pRefKF->NLeft - pRefKF->NRight - pRefKF->NSideLeft]
                    .octave;
    }
...
void MapPoint::PrintObservations() {
    ...
    for (map<KeyFrame *, tuple<int, int, int, int>>::iterator mit = mObservations.begin(),
...
void MapPoint::PreSave(set<KeyFrame *> &spKF, set<MapPoint *> &spMP) {
    ...
    for (std::map<KeyFrame *, std::tuple<int, int, int, int>>::const_iterator
