            ...
            if (!pKFi->isBad()) {
                ...
                if (pKFi->mpCamera4) {
                    ...
                    if (siderightIndex != -1 && siderightIndex < pKFi->mvKeysSideRight.size()) {
                        siderightIndex =
                            siderightIndex - pKFi->NLeft - pKFi->NRight - pKFi->NSideLeft;
            ...
            if (pMP) {
                ...
                // SLAM with respect a rigid body
                    ...
                    if (i < pFrame->Nleft) { // Left camera observation
                        ...
                        i >= pFrame->Nleft + pFrame->Nright &&
                        i < pFrame->Nleft + pFrame->Nright + pFrame->Nsideleft) {
                        ...
                        ORB_SLAM3::EdgeSE3ProjectXYZOnlySLPoseToBody *e =
                            new ORB_SLAM3::EdgeSE3ProjectXYZOnlySLPoseToBody();
                        ...
                        e->mTsll = g2o::SE3Quat(
                            pFrame->GetRelativePoseTsll().unit_quaternion().cast<double>(),
                        ...
                        ORB_SLAM3::EdgeSE3ProjectXYZOnlySRPoseToBody *e =
                            new ORB_SLAM3::EdgeSE3ProjectXYZOnlySRPoseToBody();
                        ...
                        e->mTsrl = g2o::SE3Quat(
                            pFrame->GetRelativePoseTsrl().unit_quaternion().cast<double>(),
...
void Optimizer::LocalBundleAdjustment(
    ...
    for (list<MapPoint *>::iterator lit = lLocalMapPoints.begin(), lend = lLocalMapPoints.end();
        ...
        for (map<KeyFrame *, tuple<int, int, int, int>>::iterator mit = observations.begin(),
                                                                  mend = observations.end();
             mit != mend;
             mit++) {
            KeyFrame *pKFi = mit->first;
            ...
            if (!pKFi->isBad() && pKFi->GetMap() == pCurrentMap) {
                ...
                if (pKFi->mpCamera3) {
                    ...
                    if (sideleftIndex != -1) {
                        ...
                        ORB_SLAM3::EdgeSE3ProjectXYZSLToBody *e =
                            new ORB_SLAM3::EdgeSE3ProjectXYZSLToBody();
                        ...
                        e->mTsll = g2o::SE3Quat(
                            Tsll.unit_quaternion().cast<double>(),
                            Tsll.translation().cast<double>());
                ...
                if (pKFi->mpCamera4) {
                    ...
                    if (siderightIndex != -1) {
                        ...
                        ORB_SLAM3::EdgeSE3ProjectXYZSRToBody *e =
                            new ORB_SLAM3::EdgeSE3ProjectXYZSRToBody();
                        ...
                        e->mTsrl = g2o::SE3Quat(
                            Tsrl.unit_quaternion().cast<double>(),
                            Tsrl.translation().cast<double>());
    ...
    vToErase.reserve(
        vpEdgesMono.size() + vpEdgesBody.size() + vpEdgesMono_SL.size() + vpEdgesMono_SR.size() +
        vpEdgesStereo.size());
    ...
    for (int i = 0; i < N; i++) {
        ...
        if (pKFi->bImu && pKFi->mPrevKF->bImu && pKFi->mpImuPreintegrated) {
            ...
            if (i == N - 1 || bRecInit) {
                ...
                // one linking the last optimizable keyframe inside of the local window and the
                // first fixed keyframe out. The information matrix for this measurement is also
                // downweighted. This is done to avoid accumulating error due to fixing variables.
                g2o::RobustKernelHuber *rki = new g2o::RobustKernelHuber;
            ...
            if (!pKFi->isBad() && pKFi->GetMap() == pCurrentMap) {
                ...
                if (pKFi->mpCamera3) {
                    ...
                    if (sideleftIndex != -1) {
                        ...
                        // Add here uncerteinty
                        const float unc2 = pKFi->mpCamera->uncertainty2(
                            obs); // TODO: check if need to change here
...
Eigen::MatrixXd
Optimizer::Marginalize(const Eigen::MatrixXd &H, const int &start, const int &end) {
    // Goal
    // a  | ab | ac       a*  | 0 | ac*
    // ba | b  | bc  -->  0   | 0 | 0
    // ca | cb | c        ca* | 0 | c*
...
void Optimizer::LocalBundleAdjustment(
    ...
    Verbose::PrintMess(
        "[BA]: Second optimization, there are " + to_string(badMonoMP) + " monocular and " +
...
void Optimizer::MergeInertialBA(
    ...
    for (int i = 0; i < N; i++) {
        ...
        for (vector<MapPoint *>::iterator vit = vpMPs.begin(), vend = vpMPs.end(); vit != vend;
             vit++) {
            // Using mnBALocalForKF we avoid redundance here, one MP can not be added several times
            // to lLocalMapPoints
            MapPoint *pMP = *vit;
            if (pMP)
                if (!pMP->isBad())
    ...
    for (int i = 0; i < N; i++) {
        ...
        if (!pKFi->mPrevKF) {
            Verbose::PrintMess(
                "NOT INERTIAL LINK TO PREVIOUS FRAME!!!!",
                Verbose::VERBOSITY_NORMAL);
            continue;
        }
        if (pKFi->bImu && pKFi->mPrevKF->bImu && pKFi->mpImuPreintegrated) {
...

    {
        ...
        for (int i = 0; i < N; i++) {
            ...
            if (pMP) {
                ...
                if (bRight && (i >= Nleft + Nright && i < Nleft + Nright + Nsideleft) &&
