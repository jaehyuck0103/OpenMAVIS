...
void Frame::ExtractORB(
    ...
    case 2: {
        if (enableCamera)
            monoSideLeft = (*mpORBextractorSideLeft)(
                im,
                cv::Mat(),
                mvKeysSideLeft,
                mDescriptorsSideLeft,
                vLapping);
        else {
    ...
    case 3: {
        if (enableCamera)
            monoSideRight = (*mpORBextractorSideRight)(
                im,
                cv::Mat(),
                mvKeysSideRight,
                mDescriptorsSideRight,
                vLapping);
        else {
...
Sophus::SE3f Frame::GetRelativePoseTsll() { return mTsll; }

Sophus::SE3f Frame::GetRelativePoseTlsl() { return mTlsl; }

Eigen::Matrix3f Frame::GetRelativePoseTlsl_rotation() { return mTlsl.rotationMatrix(); }

Eigen::Vector3f Frame::GetRelativePoseTlsl_translation() { return mTlsl.translation(); }

// left to side right poses
Sophus::SE3f Frame::GetRelativePoseTsrl() { return mTsrl; }

Sophus::SE3f Frame::GetRelativePoseTlsr() { return mTlsr; }

Eigen::Matrix3f Frame::GetRelativePoseTlsr_rotation() { return mTlsr.rotationMatrix(); }

Eigen::Vector3f Frame::GetRelativePoseTlsr_translation() { return mTlsr.translation(); }
...
bool Frame::isInFrustum(MapPoint *pMP, float viewingCosLimit) {
    ...
    if (Nleft == -1) {
        ...
        if (Nsideleft == -1 && Nsideright == -1) {
            ...
            return (
                pMP->mbTrackInView || pMP->mbTrackInViewR || pMP->mbTrackInViewSL ||
                pMP->mbTrackInViewSR);
        }
    }
}
...
vector<size_t> Frame::GetFeaturesInArea(
    ...
    const int minLevel,
    const int maxLevel,
    const int selectedCamera) const {
    ...
    for (int ix = nMinCellX; ix <= nMaxCellX; ix++) {
        for (int iy = nMinCellY; iy <= nMaxCellY; iy++) {
            vector<size_t> vCell =
                (selectedCamera == 0)   ? mGrid[ix][iy]
                : (selectedCamera == 1) ? mGridRight[ix][iy]
                : (selectedCamera == 2) ? mGridSideLeft[ix][iy]
                : (selectedCamera == 3)
                    ? mGridSideRight[ix][iy]
                    : (throw std::runtime_error("Error: selectedCamera value is not valid."),
                       vector<size_t>());
...
void Frame::ComputeMultiFishEyeMatches() {
    ...
    if (mDescriptors.rows != 0 && mDescriptorsRight.rows != 0) {
        ...
        // TODO: for diffrent setup, sideward camera may have sufficient overlap for stereo
        // matching
        mvLeftToRightMatch = vector<int>(Nleft, -1);
        mvRightToLeftMatch = vector<int>(Nright, -1);
