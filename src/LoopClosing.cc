            ...
            if (bFindedRegion && mbActiveLC) {
                if (mbMergeDetected) {
                    if ((mpTracker->mSensor == System::IMU_MONOCULAR ||
                         ...
                         mpTracker->mSensor == System::IMU_MULTI) &&
                        ...
                        case 1: {
                            ...
                            g2o::Sim3 Slr(
                                Tlr.unit_quaternion().cast<double>(),
                                Tlr.translation().cast<double>(),
                                1.0);
                        ...
                        case 2: {
                            ...
                            g2o::Sim3 Slsl(
                                Tlsl.unit_quaternion().cast<double>(),
                                Tlsl.translation().cast<double>(),
                                1.0);
                        ...
                        case 3: {
                            ...
                            g2o::Sim3 Slsr(
                                Tlsr.unit_quaternion().cast<double>(),
                                Tlsr.translation().cast<double>(),
                                1.0);
...
bool LoopClosing::DetectAndReffineSim3FromLastKF(
    ...
    if (nNumProjMatches >= nProjMatches) {
        ...
        int numOptMatches = Optimizer::OptimizeSim3(
            ...
            mHessian7x7,
            true,
            mCameraPair.cameraID1,
            mCameraPair.cameraID2);
    ...
    for (KeyFrame *pKFi : vpBowCand) {
        ...
        if (numBoWMatches >= nBoWMatches) // TODO pick a good threshold
        {
            ...
            if (bConverge) {
                ...
                std::cout << "There are " << vpKeyFrames.size()
                          << " KFs which view all the mappoints" << std::endl;
                ...
                g2o::Sim3 gSmw(
                    pMostBoWMatchesKF->GetRotation(cameraID2).cast<double>(),
                    pMostBoWMatchesKF->GetTranslation(cameraID2).cast<double>(),
                    1.0);
                g2o::Sim3 gScw =
                ...
                if (numProjMatches >= nProjMatches) {
                    ...
                    if (numOptMatches >= nSim3Inliers) {
                        ...
                        if (numProjOptMatches >= nProjOptMatches) {
                            ...
                            int numOptMatches2 = Optimizer::OptimizeSim3(
                                mpCurrentKF,
                                pKFi,
                                vpMatchedMP,
                                gScm,
                            ...
                            while (nNumKFs < 3 && j < vpCurrentCovKFs.size()) {
                                ...
                                Sophus::SE3d mTjc = (pKFj->GetPose(cameraID1) *
                                                     mpCurrentKF->GetPoseInverse(cameraID1))
                                                        .cast<double>();
...
int LoopClosing::FindMatchesByProjection(
    ...
    vpMatchedMapPoints.resize(
        pCurrentKF->GetMapPointMatches().size(),
        static_cast<MapPoint *>(NULL));
...
void LoopClosing::MergeLocal() {
    ...
    // Flag that is true only when we stopped a running BA, in this case we need relaunch at the
    // end of the merge
    ...
    {
        std::cout << "[Merge]: " << std::to_string(numPointsWithCorrection) << " points removed
    from Ma due to its reference KF is not in welding area" << std::endl; std::cout << "[Merge]: Ma
    has " << std::to_string(spLocalWindowMPs.size()) << " points" << std::endl;
    }*/
    ...
    while (pNewChild) {
        pNewChild->EraseChild(pNewParent); // We remove the relation between the old parent and the
                                           // new for avoid loop
    ...
    if (vpCurrentMapKFs.size() == 0) {
        ...
        // Optimize graph (and update the loop position for each element form the begining to the
        // end)
        if (mpTracker->mSensor != System::MONOCULAR) {
            Optimizer::OptimizeEssentialGraph(
                mpCurrentKF,
...

        {
            ...
            unique_lock<mutex> currentLock(
                pCurrentMap
                    ->mMutexMapUpdate); // We update the current map with the Merge information
            unique_lock<mutex> mergeLock(
                pMergeMap->mMutexMapUpdate); // We remove the Kfs and MPs in the merged area from
                                             // the old map
