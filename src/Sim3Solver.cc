...
Sim3Solver::Sim3Solver(
    ...
    for (auto const &camPair_count : camPairCount)
        camPairsCountVec.push_back(
            std::pair<std::pair<int, int>, int>(camPair_count.first, camPair_count.second));
...
Eigen::Matrix4f
Sim3Solver::iterate(int nIterations, bool &bNoMore, vector<bool> &vbInliers, int &nInliers) {
    bNoMore = false;
...
void Sim3Solver::ComputeSim3(Eigen::Matrix3f &P1, Eigen::Matrix3f &P2) {
    ...
    if (vec.norm() != 0)
        vec = 2 * ang * vec / vec.norm(); // Angle-axis representation. quaternion angle is the
                                          // half
