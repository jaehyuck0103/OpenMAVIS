...
FrameDrawer::FrameDrawer(Atlas *pAtlas, const int sensor)
    : both(false),
      mpAtlas(pAtlas),
      mSensor(sensor) {
...
cv::Mat FrameDrawer::DrawSideRightFrame(float imageScale) {
    ...
    } else if (state == Tracking::OK) // TRACKING
    {
        ...
        const int SideLeftBoundary =
            mvCurrentKeys.size() + mvCurrentKeysRight.size() + mvCurrentKeysSideLeft.size();
...
void FrameDrawer::Update(Tracking *pTracker) {
    ...
    if ((mSensor == System::STEREO || mSensor == System::IMU_STEREO ||
        ...
        N = mvCurrentKeys.size() + mvCurrentKeysRight.size() + mvCurrentKeysSideLeft.size() +
            mvCurrentKeysSideRight.size();
    } else {
